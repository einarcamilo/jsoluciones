...
export class Pilas {
    constructor() {
        this.pilas = new Array();
        this.limitElements = 9;
        this.Eval = new Evaluation();
        this.Util = new Utilities();
    }
    //#region Explora
    GetExplora(user, quantity = 0) {
        if (quantity === 0) {
            $('.main-pilas').empty();
        }
        $.ajax({
    ...
    DrawExplora(quantity) {
        $(".main-pilas").find('.main-more').remove();
        if (this.pilas.length <= 0) {
            $(".main-pilas").append(`<div class="main-more row">
    ...
    GetById(id, exist, old) {
        if (exist) {
            let pilas = this.pilas.filter(p => p.Id === id);
            if (pilas.length > 0) {
                this.DrawDetail(pilas[0]);
            }
            else {
                this.GetNew(id);
            }
    ...
    HTMLHistoricalInternal(JsonHistorico) {
    ...
    HTMLHistoricalWeeks(JsonHistorico) {
        let yy, mm, ww;
        for (let pilas of JsonHistorico) {
            yy = pilas.Year;
            for (let months of pilas.Months) {
    ...
    HTMLHistoricalContent(JsonHistorico) {
        let yy, mm, ww;
        for (let pilas of JsonHistorico) {
            yy = pilas.Year;
            for (let months of pilas.Months) {
    ...
    DateLong(date) {
        ...
        let d = new Date(date), month = months[d.getMonth()], year = d.getFullYear(), day = (d.getDate() < 10) ? "0" + d.getDate() : d.getDate();
