    ...
    GetMain() {
        if (this.MenuPrincipal.length <= 0) {
            $.ajax({
                ...
                success: info => this.MenuPrincipal = info,
                complete: () => { this.DrawMain(); setTimeout(() => this.MenuPrincipal = new Array(), 300000); }
            });
        }
        else {
            this.DrawMain();
