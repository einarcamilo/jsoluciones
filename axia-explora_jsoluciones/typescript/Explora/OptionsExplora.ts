...
export class OptionsExplora{
  ...
  private Pilas: Pilas;
  private MenuExplora: MenuExplora;
  ...
  DetailPilas(id: number, exist: boolean, old: number = 0){
  ...
  DeployModalMenu(type: string, url: string, title: string){
  ...
  ReturnInitial(){
    ...
    if(buscador.length>0){ $(".main-buscador").slideDown(); }
    else if(historico.length>0){ $(".main-historico").slideDown(); }
    else { $(".main-pilas").slideDown(); }
  }
  private OrderAnchored(element: any, value: number){
    ...
    $(".main-pilas").find('.element-pilas').each(function(){
      ...
      if($(this).find(".btn-anchoring").hasClass("btn-anchoring-active")){ initial++; }
      if(breaking == 3){ return false; }
      breaking++;
    });
