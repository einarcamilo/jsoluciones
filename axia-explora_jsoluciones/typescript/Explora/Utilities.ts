export class Utilities{
  ...
  OrderImg(container:string){
    $("."+container).find('img').each(function(){
      $(this).removeAttr("width height").addClass("img-fluid");
      let src = $(this).attr("src");
      if(src != undefined) { 
        if(src.indexOf("libs/imagePilas/")==0)$(this).attr({src: "http://iagregado/intranetva/datagrid/dbgrid/pilas_online/" + src}); 
      }
    });
  }
}