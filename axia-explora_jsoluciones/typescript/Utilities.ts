...
const SerializeData=(form: HTMLFormElement)=>{
  let several = new Array();
  let obj: { [key: string]: any } = {};
  const data = new FormData(form);
  for(let i of data){ 
    let element = document.getElementsByName(i[0])[0] as HTMLInputElement;
    let tag = element.type;
    if(tag === "checkbox"){several.push(i[0]);}
  }
  for(let i of data){ 
...
const CompareDate = (date: string, actual: string)=>{
